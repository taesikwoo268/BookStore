...
@Component
@Slf4j
public class CancelledStateTransition implements OrderStateTransition {

    @Override
    public boolean canTransition(OrderStatus currentStatus, OrderStatus targetStatus) {
        return false; // Không thể transition từ CANCELLED
    }

    @Override
    public Order execute(Order order, OrderStatus targetStatus, String reason) {
        throw new RuntimeException("Cannot transition from CANCELLED status");
    }
}